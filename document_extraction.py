...
CHECKPOINT_MODEL_PATH = "C:\\Users\\vkdnj\\Zolph\\models\\model_mbv3_iou_mix_2C049.pth"
...
def process_image(name, trained_model=trained_model):
    DATA_DIR = "C:\\Users\\vkdnj\\Zolph\\comics\\Pages"
    ...
    document = extract(image_true=image[:, :, ::-1], trained_model=trained_model)
        
    if document.dtype == 'float64':
        document = document.astype('float32')  # float64에서 float32로 변환
    # 이제 BGR로 변환
    document_bgr = cv2.cvtColor(document, cv2.COLOR_RGB2BGR)
    
    CROP_DATA_DIR = "C:\\Users\\vkdnj\\Zolph\\comics\\proc_images"

    output_path = os.path.join(CROP_DATA_DIR, "cropped_" + name + ".jpg")
    cv2.imwrite(output_path, document_bgr)
    
    return "cropped_" + name
